...
static void csense_start(void)
{
    ...
    NRF_LOG_INFO("csense add button.");
    err_code = nrf_csense_add(&m_button);
    APP_ERROR_CHECK(err_code);

    NRF_LOG_INFO("csense add slider.");
...
int main(void)
{
    ...
    leds_init();
    ret_code_t err_code;
    NRF_LOG_INFO("config clock");
    err_code = clock_config();
    APP_ERROR_CHECK(err_code);
    NRF_LOG_INFO("config clock complete");
    timers_init();
    buttons_init();
    ...
    // Start execution.
    NRF_LOG_INFO("csense example started.");
    advertising_start();
